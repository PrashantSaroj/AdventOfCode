...
class Tetris:
    ...
    def findPattern(self):
        # initial part of board does not repeat
        # cycles occur at 1715 length
        cycleLength = 1715
        cycleHeight = sum(self.heightIncr[cycleLength:2*cycleLength])

        repeatedCalc = (10**12 // cycleLength - 1) * cycleHeight
        restHeights = sum(
            self.heightIncr[:(10**12 % cycleLength) + cycleLength])
        print(repeatedCalc + restHeights)
    ...
    def simulate(self):
        ...
        for i in range(self.ROCKS_COUNT):
            ...
            self.heightIncr.append(max(0, currRock.getTop()-currTop))
