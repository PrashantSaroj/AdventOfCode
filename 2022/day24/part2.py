...
class ValleySim:
    ...
    def simulate(self):
        # go to evac point
        t1 = self.calcMinTime(self.src, -1) + 1
        # go back to valley entrance
        self.memo = {}
        self.src = (self.rowCount, self.colCount-1)
        self.dst = (0, 0)
        t2 = self.calcMinTime(self.src, t1) + 1

        # go to evac again
        self.memo = {}
        self.src = (-1, 0)
        self.dst = (self.rowCount-1, self.colCount-1)
        t3 = self.calcMinTime(self.src, t2)
        print(t1, t2, t3)
